...
- `frontend/` — Angular 19 SPA (NgModule-based, not standalone components) with Angular Material, Bootstrap 5, and FullCalendar.
...
### Frontend: NgModule + role-based lazy-loaded feature modules

Despite using Angular 19, this app is **not** using standalone components as the primary pattern — `AppModule` (`app.module.ts`) declares/imports most components, dialogs, and Material modules, while `app.config.ts`'s `appConfig` is merged in as an extra provider. Guards and interceptors, however, are written as standalone functions (`CanActivateFn`, `HttpInterceptorFn`).

Routing (`app.routes.ts`) splits into:
- Public routes (login, signup, role selection, password reset, email verification, etc.), and
- Two lazy-loaded, role-specific feature modules gated by `canMatch: [completeAuthGuard]`: `mod-organizer/` at `organizer/dashboard/**` and `mod-attendee/` at `attendee/dashboard/**`, each with its own `*-routing.module.ts`.
...
- `completeAuthGuard` — gates the lazy feature modules, checking both login state and role assignment via `DashboardCacheService`.
